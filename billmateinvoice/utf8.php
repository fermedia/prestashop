...
//		$string1 = utf8_strtolower( preg_replace('/([\s+])/', '', Encoding::fixUTF8($string1) ));
	...
	class GoogleTranslateApi{ 
		...
		function makeCallUrl(){ 
			echo $this->CallUrl = $this->BaseUrl . "?v=" . $this->Version . "&q=" . urlencode($this->Text) . "&langpair=" . $this->FromLang . "%7C" . $this->ToLang; 
		} 
	...
	function billmate_translate_country($text='',$to='SV'){
		global $countries;
		$to = strtoupper($to);
		$text = strtoupper($text);
		if( isset($countries[$to]) && isset($countries[$to][$text])){
			return $countries[$to][$text];
		}
		return $text;
	}
}
