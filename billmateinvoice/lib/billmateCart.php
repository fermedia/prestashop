 ...
 *
 */
require __DIR__.'/preOrder.php';
class billmateCart extends PaymentModule{
	public $active = true;
	public $order = null;
	public $name = 'billmatecardpay';
	public function __construct($name = 'billmatecardpay'){
		$this->name = $name;
	}
	...
	function completeOrder($extra = array(), $id_cart=0)
	{
		...
		$prevOrder = OrderHistory::getLastOrderState($this->id);
		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			foreach ($package_list as $id_address => $packageByAddress)
				foreach ($packageByAddress as $id_package => $package)
				{
					...
					$order->id_cart = 0;//(int)$this->context->cart->id
